...
def main():
    ...
    # Commit current changes so the checkout is clean for verification and packaging
    subprocess.run(["git", "add", "tools/", "experiments/"], capture_output=True)
    subprocess.run(["git", "commit", "-m", "fix(runner): stage and commit runner and mutation fixes for clean checkout"], capture_output=True)

    os.environ["SOVEREIGN_SECRET"] = "sovereign_ephemeral_key_999"

    # 1. Run package now that checkout is clean
    subprocess.run(["python3", "tools/cell_runner.py", "package"], check=True, capture_output=True)

    # 2. Execute real signed mutation for P11
    ...
    # 3. Update evidence/p11/resurrection_evidence.json
    ...
    # 6. Test dirty checkout rejection
    ...
    # 7. Test secret handling (missing secret)
    ...
    # Final commit of evidence and master plan update
    subprocess.run(["git", "add", "evidence/", "SOVEREIGN_SEED_MASTER_EXECUTION_PLAN.md"], check=True)
    subprocess.run(["git", "commit", "-m", "fix(p11): update resurrection evidence and master plan statuses"], check=True)
